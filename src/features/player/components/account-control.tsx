...
export default function AccountControl() {
  ...
  return (
    <>
      ...
      >
        <span className="grid size-[30px] place-items-center rounded-full bg-jade-500 text-sm font-black" aria-hidden="true">
            ...
            {identity ? (
              <>
                ...
                <div className="account-actions mt-4 grid grid-cols-2 gap-2">
                  ...
                  <button
                    type="button"
                    className={`${ACCOUNT_ACTION_BUTTON_CLASS} account-signout text-city-900`}
                    onClick={() => {
                      setBusy(true);
                      void signOut()
                        .then(() => setOpen(false))
                  ...
                  <button
                    type="button"
                    className={`${ACCOUNT_ACTION_BUTTON_CLASS} account-delete-progress col-span-2 !border-city-500/25 !bg-city-500/5 text-city-900`}
                    onClick={() => {
                ...
                {deleteConfirmOpen ? (
                  <section
                    className="account-delete-confirm mt-3.5 rounded-xl border border-city-500/20 bg-city-500/5 p-3.5 text-city-900"
                    role="alertdialog"
                ...
                {formMessage ? <p className="account-form-success mt-3 rounded-xl bg-jade-500/10 px-3 py-2.5 text-xs leading-5 text-jade-700">{formMessage}</p> : null}
                {formError ? <p className="account-form-error mt-3 rounded-xl bg-city-500/10 px-3 py-2.5 text-xs leading-5 text-city-900">{formError}</p> : null}
              </>
            ) : (
              <>
                ...
                <div className="account-tabs my-5 grid grid-cols-2 rounded-xl bg-paper-deep p-1" role="tablist" aria-label="账户操作">
                  <button
                    ...
                    className={`${ACCOUNT_TAB_CLASS} ${mode === "signin" ? "bg-card text-jade-700 shadow-sm" : ""}`}
                  ...
                  <button
                    ...
                    className={`${ACCOUNT_TAB_CLASS} ${mode === "signup" ? "bg-card text-jade-700 shadow-sm" : ""}`}
                ...
                <form className="account-form grid gap-2" onSubmit={submit}>
                  ...
                  <input
                    ...
                    required
                  />
                  <button className="mt-2.5 grid min-h-12 cursor-pointer place-items-center rounded-xl border-0 bg-jade-500 px-4 py-2.5 text-xs font-black text-white disabled:cursor-wait disabled:opacity-60" type="submit" disabled={busy || !initialized}>
