...
export const GAUNTLET_ROUTE_TARGET = 10;
...
export const GAUNTLET_FIXED_TARGETS: Partial<Record<GauntletLevel, number>> = {
  ...
  [GAUNTLET_LEVEL_ID.NEIGHBOR_CHAIN]: GAUNTLET_ROUTE_TARGET,
  [GAUNTLET_LEVEL_ID.CITY_UNDERCOVER]: 20,
  [GAUNTLET_LEVEL_ID.REGION_MAP]: GAUNTLET_REGION_MAP_MAX_TARGET,
  [GAUNTLET_LEVEL_ID.PROVINCE_SHORTEST_ROUTE]: GAUNTLET_ROUTE_TARGET,
  ...
  [GAUNTLET_LEVEL_ID.CITY_SHORTEST_ROUTE]: GAUNTLET_ROUTE_TARGET,
...
const CITY_ROUTE_UNAVAILABLE_PROVINCE_CODES = new Set([
  "110000",
  "120000",
  "310000",
  "500000",
...
export const ALL_CITY_ROUTE_PROVINCE_CODES = PROVINCES
  .filter((item) => !CITY_ROUTE_UNAVAILABLE_PROVINCE_CODES.has(item.code))
  .map((item) => item.code);
