  ...
  const renderDetailContent = () => {
      ...
      return (
        <div className="knowledge-neighbor-layout grid min-h-[650px] grid-cols-[260px_minmax(0,1fr)] overflow-hidden rounded-[22px] border border-jade-500/20 bg-paper-100/85 max-lg:grid-cols-1">
          <aside className="border-r border-jade-500/15 bg-jade-200 px-5 py-6 max-lg:border-b max-lg:border-r-0">
            <p className="mb-3.5 mt-0 text-[9px] font-black tracking-[.1em] text-moss-500">选择中心省份</p>
            <div className="knowledge-province-selector grid grid-cols-3 gap-1.5 max-lg:grid-cols-6 max-sm:grid-cols-4">
              {provinces.map((province) => (
                <button
                  key={province.code}
                  type="button"
                  className={`min-h-[35px] cursor-pointer rounded-lg border p-[5px] text-[9px] ${province.code === selectedNeighborProvince?.code ? "border-jade-500 bg-jade-500 font-black text-white" : "border-jade-500/15 bg-white/55 text-ink-600"}`}
          ...
          <section className="knowledge-neighbor-stage grid place-content-center place-items-center gap-[34px] [background-image:radial-gradient(circle_at_center,rgba(45,125,95,.1),transparent_20rem),linear-gradient(rgba(53,66,56,.025)_1px,transparent_1px),linear-gradient(90deg,rgba(53,66,56,.025)_1px,transparent_1px)] [background-size:auto,25px_25px,25px_25px] p-10 max-md:p-6">
            <div className="neighbor-orbit grid w-[min(740px,100%)] place-items-center gap-[30px]" aria-label={`${selectedNeighborProvince?.name}的陆地邻省`}>
              <article className="neighbor-center grid size-[155px] place-content-center place-items-center rounded-[50%_50%_50%_20%] border-[5px] border-double border-white/45 bg-jade-500 text-center text-white shadow-[0_20px_40px_rgba(23,84,62,.2)]">
                ...
                <h3 className="my-1 font-serif text-[27px]">{selectedNeighborProvince?.shortName}</h3>
                <p className="m-0 text-[9px] opacity-80">{plainPlaceName(provinceCapitals[selectedNeighborProvince?.code])}</p>
              </article>
              <div className="neighbor-satellites flex flex-wrap justify-center gap-[9px]">
                {selectedNeighborCodes.length > 0 ? selectedNeighborCodes.map((code, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className="row-span-2 grid size-[22px] place-items-center rounded-full bg-jade-300 font-numeric text-[8px] text-jade-700">{index + 1}</span>
                      <strong className="text-[11px]">{neighbor.shortName}</strong>
                      <small className="text-[7px] text-stone-500">{plainPlaceName(provinceCapitals[code])}</small>
                    </button>
                  );
                }) : (
            ...
            <div className="knowledge-neighbor-mnemonic flex w-[min(600px,100%)] items-center gap-[13px] rounded-[13px] border border-jade-500/20 bg-jade-200/90 px-[18px] py-3.5">
              ...
              <p className="m-0 grid gap-1 text-[9px] text-ink-600">
          ...
          <ol className="knowledge-count-ranking m-0 grid list-none gap-[7px] p-0">
            {sortedCounts.map((item, index) => (
              <li className="grid min-h-[58px] grid-cols-[34px_90px_minmax(100px,1fr)_55px_minmax(260px,.9fr)] items-center gap-[13px] rounded-[11px] border border-gold-700/15 bg-paper-100/85 px-3.5 py-2.5 max-lg:grid-cols-[34px_90px_minmax(100px,1fr)_55px] max-sm:grid-cols-[30px_50px_1fr_50px]" key={item.code}>
                ...
                <b className="text-right font-numeric text-xl text-gold-800">{item.cityCount}<small className="ml-[3px] text-[8px]">座</small></b>
                <p className="m-0 text-[8px] leading-[1.5] text-stone-600 max-lg:col-span-full">{item.explanation}</p>
              </li>
            ))}
          </ol>
        </div>
          ...
          {RIVER_KNOWLEDGE.map((river) => {
            ...
            <article className={`knowledge-river-card overflow-hidden rounded-[22px] border p-7 ${tone.card}`} key={river.id}>
              <header className="flex items-end justify-between gap-[30px] max-md:block">
                <div>
                  <p className={`m-0 text-[9px] font-black tracking-[.12em] ${tone.text}`}>{river.label}</p>
                  <h3 className={`mb-0 mt-[5px] font-serif text-[42px] ${tone.text}`}>{river.name}</h3>
                </div>
                <dl className="m-0 flex gap-[26px] max-sm:mt-4 max-sm:grid max-sm:grid-cols-3 max-sm:gap-2">
                  <div className="grid gap-[5px]"><dt className="text-[8px] text-stone-500">源头</dt><dd className="m-0 text-[11px] font-black">{river.source}</dd></div>
                  <div className="grid gap-[5px]"><dt className="text-[8px] text-stone-500">入海</dt><dd className="m-0 text-[11px] font-black">{river.mouth}</dd></div>
                  <div className="grid gap-[5px]"><dt className="text-[8px] text-stone-500">长度</dt><dd className="m-0 text-[11px] font-black">{river.length}</dd></div>
                </dl>
              </header>
              <section className={`river-mnemonic mt-[25px] flex items-center gap-[13px] rounded-xl border border-dashed px-4 py-[13px] ${tone.mnemonic}`}>
                <span className={`rounded-md px-[7px] py-1 text-[8px] font-black text-white ${tone.badge}`}>口诀</span>
              ...
              >
                {river.provinceCodes.map((code, index) => (
                  <div className="z-[1] grid min-w-[66px] justify-items-center gap-[7px] px-[3px]" key={code}>
                    <span className={`grid size-[42px] place-items-center rounded-full border-[5px] border-paper-200 font-numeric text-[9px] text-white ${tone.marker}`}>{index + 1}</span>
              ...
              <div className={`river-cities mt-6 flex items-baseline gap-[18px] border-t pt-[18px] ${tone.divider}`}>
                <span className="shrink-0 text-[8px] font-black text-stone-500">代表城市节点（非完整名录）</span>
                <p className="m-0 text-[10px] leading-[1.7] text-ink-600">{river.representativeCities.join(" · ")}</p>
              </div>
              <a className={`mt-3.5 inline-block text-[8px] font-extrabold no-underline ${tone.link}`} href={river.sourceUrl} target="_blank" rel="noreferrer">资料来源：{river.sourceLabel} ↗</a>
            </article>
            );
          })}
        </div>
          ...
          {groups.map((group, groupIndex) => (
            <article className="grid min-h-[330px] content-start rounded-[19px] border border-jade-500/20 bg-paper-100/90 p-6 [background-image:radial-gradient(circle_at_100%_0%,rgba(45,125,95,.08),transparent_16rem)]" key={group.title}>
              <header className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-[13px]">
                ...
                <div><h3 className="m-0 font-serif text-[19px]">{group.title}</h3><p className="mb-0 mt-1.5 text-[9px] leading-[1.6] text-ink-500">{group.description}</p></div>
              </header>
              ...
              <p className="territory-memory-tip mt-auto grid gap-[5px] border-t border-black/[.13] pt-[19px] text-[9px] leading-[1.6] text-ink-500"><b className="text-[8px] tracking-[.1em] text-jade-700">记忆抓手</b>{memoryTips[groupIndex]}</p>
            </article>
          ))}
        </div>
      );
        ...
        <div className="knowledge-confusable-grid grid grid-cols-3 gap-[13px] max-lg:grid-cols-2 max-sm:grid-cols-1">
          {filteredPairs.map((pair, index) => (
            <article className="rounded-[17px] border border-city-500/20 bg-paper-100/90 p-[18px]" key={pair.id}>
              <header className="mb-[13px] flex justify-between font-numeric text-[9px] text-stone-500"><span>辨析 {String(index + 1).padStart(2, "0")}</span><b className="text-xs text-city-500">VS</b></header>
              <div className="grid grid-cols-2 gap-2">
                {[pair.left, pair.right].map((city, sideIndex) => (
                  <section className={`grid min-h-[115px] place-content-center place-items-center rounded-xl border p-3.5 ${sideIndex === 0 ? "border-city-500/15 bg-clay-100" : "border-atlas-500/15 bg-atlas-200"}`} key={`${pair.id}:${sideIndex}`}>
                    ...
                    <p className="m-0 text-[8px] font-extrabold text-ink-500">{city.provinceShort}</p>
                  </section>
                ))}
              </div>
              <p className="mb-0 mt-3 text-[9px] leading-[1.7] text-ink-500"><b className="mr-2 text-[8px] tracking-[.1em] text-city-900">记忆钩子</b>{pair.memoryTip}</p>
            </article>
          ))}
        </div>
      );
    ...
    return (
      <div className="knowledge-reading-layout grid grid-cols-[minmax(260px,.48fr)_minmax(0,1fr)] gap-5 max-lg:grid-cols-1">
        <section className="knowledge-reading-intro sticky top-28 grid min-h-[470px] self-start content-center rounded-[23px_23px_23px_8px] bg-gold-800 [background-image:radial-gradient(circle_at_100%_0%,rgba(255,255,255,.16),transparent_16rem)] p-[34px] text-gold-100 shadow-[0_20px_44px_rgba(114,84,18,.18)] max-lg:static max-lg:min-h-0">
          <span className="text-[9px] font-black tracking-[.14em]">读图五步法</span>
          <h3 className="my-[18px] font-serif text-[clamp(27px,3vw,39px)] leading-tight">大范围 → 小范围<br />位置 → 边界 → 路线</h3>
          <p className="m-0 text-[11px] leading-[1.8] opacity-85">地图题不是只靠死记轮廓。把观察顺序固定下来，陌生题也能用排除法解决。</p>
          <button className="mt-[27px] cursor-pointer rounded-[10px] border-0 bg-gold-200 px-3.5 py-3 text-[10px] font-black text-gold-800" type="button" onClick={onOpenAtlas}>打开全国车牌图鉴练读图</button>
        </section>
        <ol className="knowledge-tip-list m-0 grid list-none gap-3 p-0">
          {MAP_READING_TIPS.map((tip, index) => (
            <li className="grid min-h-[150px] grid-cols-[62px_minmax(0,1fr)] items-center gap-5 rounded-[17px] border border-gold-700/15 bg-paper-100/90 p-6" key={tip.mark}>
              ...
              <div>
                <small className="text-[8px] font-black tracking-[.1em] text-stone-500">第 {index + 1} 步</small>
                ...
                <strong className="mt-2.5 inline-block rounded-md bg-gold-300 px-2 py-[5px] text-[9px] text-gold-800">{tip.mnemonic}</strong>
              </div>
            </li>
          ))}
        </ol>
    ...
    <main className="knowledge-shell min-h-dvh bg-paper-400 [background-image:radial-gradient(circle_at_8%_5%,rgba(255,255,255,.96),transparent_27rem),radial-gradient(circle_at_90%_13%,rgba(110,78,128,.08),transparent_26rem),linear-gradient(rgba(53,66,56,.03)_1px,transparent_1px),linear-gradient(90deg,rgba(53,66,56,.03)_1px,transparent_1px)] [background-size:auto,auto,30px_30px,30px_30px] text-ink">
      <header className="knowledge-header sticky top-0 z-40 flex min-h-[78px] items-center justify-between gap-6 border-b border-black/[.14] bg-card/95 px-[max(24px,calc((100vw_-_1380px)/2))] py-3 shadow-[0_8px_30px_rgba(58,47,32,.06)] backdrop-blur-xl max-sm:min-h-16 max-sm:gap-2 max-sm:px-3 max-sm:py-2">
        <button className="knowledge-brand flex cursor-pointer items-center gap-3 border-0 bg-transparent p-0 text-left" type="button" onClick={backToCatalog}>
          ...
          <div>
            <p className="m-0 text-[8px] font-black tracking-[0.16em] text-scholar-500 max-sm:hidden">CHINA GEO KNOWLEDGE</p>
            <h1 className="m-0 font-serif text-[21px] leading-none max-sm:text-base">中国地理知识馆</h1>
          </div>
        </button>
        <div className="knowledge-header-actions flex gap-2">
          ...
          <button className="knowledge-exit min-h-10 cursor-pointer rounded-full border border-city-900 bg-city-900 px-3.5 text-[10px] font-black text-white" type="button" onClick={onExit}>返回游戏</button>
        </div>
      </header>
      ...
      {!activeCategory ? (
        ...
        <>
          <section className={`knowledge-detail-hero mx-auto grid w-[min(1380px,calc(100%_-_48px))] grid-cols-[84px_minmax(0,1fr)_minmax(280px,360px)] items-center gap-[25px] border-b pb-[38px] pt-[54px] max-md:grid-cols-[auto_1fr] max-sm:w-[calc(100%_-_24px)] max-sm:gap-3 max-sm:pb-6 max-sm:pt-7 ${DETAIL_TONE_CLASSES[activeCategory.tone].border}`}>
            ...
            <div>
              ...
              <h2 className="mb-0 mt-[7px] font-serif text-[clamp(30px,4vw,47px)] leading-[1.1] max-sm:text-2xl">{activeCategory.title}</h2>
              ...
              <div className="mt-3.5 flex flex-wrap gap-1.5">{activeCategory.levelRefs.map((levelId) => {
                ...
                return levelNumber > 0
                  ? <i className={`inline-flex rounded-full border px-[7px] py-1 text-[8px] font-extrabold not-italic ${DETAIL_TONE_CLASSES[activeCategory.tone].pill}`} key={levelId}>关联第 {levelNumber} 关</i>
                  : null;
              })}</div>
            </div>
          ...
          <section className="knowledge-detail-content mx-auto min-h-[520px] w-[min(1380px,calc(100%_-_48px))] pb-16 pt-[38px] max-sm:w-[calc(100%_-_24px)] max-sm:pb-10 max-sm:pt-6">{renderDetailContent()}</section>
          <footer className="knowledge-page-footer mx-auto flex w-[min(1380px,calc(100%_-_48px))] items-center justify-between gap-5 border-t border-black/[.13] pb-[42px] pt-6 max-sm:w-[calc(100%_-_24px)] max-sm:flex-col">
            <button className="cursor-pointer rounded-full border border-scholar-500/25 bg-scholar-100 px-[13px] py-[9px] text-[9px] font-black text-scholar-600" type="button" onClick={backToCatalog}>← 继续浏览其他知识专题</button>
            <span className="text-[8px] text-stone-500">知识来自当前关卡题库及注明的权威公开资料</span>
          </footer>
        </>
      )}
    </main>
