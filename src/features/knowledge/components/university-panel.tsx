    ...
    <div className="knowledge-stack grid gap-5">
      <div className="knowledge-memory-banner grid grid-cols-[50px_minmax(0,1fr)_auto] items-center gap-4 rounded-[17px] border border-scholar-500/20 bg-scholar-200 px-5 py-[18px] text-scholar-600 max-md:grid-cols-[50px_1fr]">
        ...
        <div>
          ...
          <p className="mb-0 mt-1 text-meta text-ink-600">北京、上海、南京、武汉、西安、成都等城市聚集较多，再补上每省的单点学校。</p>
        </div>
        <a
          className="inline-flex min-h-10 items-center justify-center rounded-full border border-current bg-white/50 px-[13px] py-2 text-compact font-black text-current no-underline max-md:col-span-2 max-md:min-h-11 max-md:justify-self-start"
        ...
        {groups.map(({ province, items }) => (
          <section className="overflow-hidden rounded-[17px] border border-scholar-500/20 bg-paper-100/90" key={province.code}>
            <header className="flex items-end justify-between gap-[15px] bg-scholar-200 px-[18px] py-4 text-scholar-600">
              <div>
                <p className="m-0 text-meta">{province.kind}</p>
                <h3 className="mb-0 mt-[3px] font-serif text-lg">{province.shortName}</h3>
              </div>
              <strong className="font-numeric text-[25px]">{items.length}<small className="ml-[3px] text-meta">所</small></strong>
            </header>
            <div className="grid px-[15px] pb-[13px] pt-2">
              {items.map((item) => (
                <article className="grid grid-cols-[38px_minmax(0,1fr)] items-center gap-2.5 border-b border-black/[.13] py-2.5 last:border-b-0" key={item.id}>
                  <span className={`university-tier grid h-[25px] w-[38px] place-items-center rounded-[7px] border font-numeric text-meta font-black ${item.tier === "985" ? "border-city-500/25 bg-city-200 text-city-600" : "border-scholar-500/25 bg-scholar-100 text-scholar-600"}`}>{item.tier}</span>
                  <div>
                    <h4 className="m-0 text-xs">{item.name}</h4>
                    <p className="mb-0 mt-1 text-meta text-stone-600">
