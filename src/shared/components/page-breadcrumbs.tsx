...
export default function PageBreadcrumbs({
  ...
  return (
    <nav
      ...
      aria-label="面包屑导航"
    >
      <ol className="m-0 flex min-h-11 min-w-0 list-none items-center gap-2 p-0 text-compact font-extrabold text-ink-500 max-sm:gap-1.5 max-sm:text-compact-mobile">
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              {index > 0 ? (
                <span className="text-compact-mobile text-stone-400 max-sm:text-meta" aria-hidden="true">/</span>
              ) : null}
              {item.href && !current ? (
                <AppLink
                ...
                <span
                  className="min-w-0 truncate text-body text-ink max-sm:text-compact"
