...
export const GAUNTLET_LEVELS: readonly GauntletLevelDefinition[] = [
  {
    ...
    target: "34 个省级行政区",
    openingFeedback: "观察轮廓，写出省级行政区名称",
    roundHeading: "看轮廓，识省份",
  },
  {
  ...
  {
    id: GAUNTLET_LEVEL_ID.ROTATED_SHAPE,
    title: "旋转轮廓",
    badge: "旋转",
    description: "省份轮廓会随机旋转，失去正常方向提示后辨认名称。",
    target: "连续答对 20 题",
    openingFeedback: "忽略旋转方向，辨认省份轮廓",
    roundHeading: "旋转之后还能认出省份吗",
  },
  {
...
const STORED_GAUNTLET_LEVEL_ID_PATTERN =
  /^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/;

/** Accepts active and retired semantic IDs, but never display numbers. */
export function isStoredGauntletLevelId(value: unknown): value is string {
  return typeof value === "string" && STORED_GAUNTLET_LEVEL_ID_PATTERN.test(value);
}

const ACTIVE_GAUNTLET_LEVEL_IDS: ReadonlySet<string> = new Set(
  GAUNTLET_LEVELS.map((level) => level.id),
);
...
export function activeGauntletCompletionCount(levelIds: readonly string[]) {
  return new Set(levelIds.filter((levelId) => ACTIVE_GAUNTLET_LEVEL_IDS.has(levelId))).size;
}
